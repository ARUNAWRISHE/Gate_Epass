...
def create_hod_request():
    try:
        # Validate file existence (approval letter)
        ...
        if '.' not in approval_letter.filename or approval_letter.filename.rsplit('.', 1)[1].lower() not in allowed_extensions:
            return jsonify({"error": "Invalid file type. Only .jpg, .jpeg, .png, .pdf are allowed"}), 400

        # Save the file with a unique filename
        filename = f"{request.form.get('event_name', 'unknown_event')}{datetime.now().timestamp()}{approval_letter.filename}"
        approval_letter.save(os.path.join(app.config['UPLOAD_FOLDER'], filename))
        ...
        # Validate file extension for appreciation letter
        if '.' not in appreciation_letter.filename or appreciation_letter.filename.rsplit('.', 1)[1].lower() not in allowed_extensions:
            return jsonify({"error": "Invalid file type for appreciation letter. Only .jpg, .jpeg, .png, .pdf are allowed"}), 400

        # Save the appreciation letter with a unique filename
        appreciation_letter_filename = f"appreciation_{request.form.get('event_name', 'unknown_event')}{datetime.now().timestamp()}{appreciation_letter.filename}"
        ...
        departmentid = request.form.get('hod_id')
        print(departmentid)
        department = HOD.query.get(departmentid).department
        print(department)
        if not department:
            return jsonify({"error": "Department is required"}), 400
        print(f"Department: {department}")

        # Get the first two letters of the department name (ignoring case)
        department_prefix = re.sub(r'[^a-zA-Z]', '', department)[:2].upper()

        # Find the latest request ID in the department
        ...
        # Check if the generated ID already exists, and increment if necessary
        while Request.query.filter_by(id=new_id).first():
            last_number += 1
            new_id = f"{department_prefix}{last_number}"

        # Get staff ID (convert to integer)
        staff_id = request.form.get('hod_id')
...
def send_otp_email(email, otp, qr_path):
    try:
        sender_email = "sanjayram2305@gmail.com"
        sender_password = "zyci yvps kyzc mrme"
...
def send_thankyou_email(email, event_name):
    try:
        sender_email = "sanjayram2305@gmail.com"  # Replace with your email
        sender_password = "zyci yvps kyzc mrme"   # Replace with your app password
...
def verify_otp():
    try:
        ...
        # Return request details including time_in
        return jsonify({
            "success": True,
            "request": {
                ...
                "time_in": request_obj.time_in  # Include time_in in the response
            }
        }), 200
